...
export default function Two(props) {
  const data1 = props.data1.slice(0, 7);
  const data2 = props.data2.slice(0, 8);
  const data = [];
  ...
  const [number, setNumber] = useLocalStorage("QuestionNoOrange1", 0);
  const [activeStep, setActiveStep] = React.useState(parseInt(number));
  const [options, setOptions] = React.useState("");
  const [next, setNext] = React.useState(true);
  const [score, setScore] = useLocalStorage("scoreorange1", 0);
  const handleNext = () => {
