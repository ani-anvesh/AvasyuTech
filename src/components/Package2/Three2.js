...
export default function Three2(props) {
  const data1 = props.data.slice(16, 23);
  const data2 = props.data1.slice(16, 24);
  const data = [];
  ...
  const [number, setNumber] = useLocalStorage("QuestionNogreen2", 0);
  ...
  const [score, setScore] = useLocalStorage("scoregreen2", 0);
  const handleNext = () => {
    ...
    setLocal({
      ...local,
      green1:
        parseInt(localStorage.getItem("scoregreen2")) > 0
          ? parseInt(localStorage.getItem("scoregreen2"))
          : 0,
    });
    if (activeStep >= 14) {
      setNext(false);
      ...
      <Paper className="root">
        ...
        <Grid container xs={12} md={12}>
          ...
          {next ? (
            ...
            >
              <Link to="/result2" style={{ textDecoration: "none" }}>
                <Button
                  variant="outlined"
                  color="primary"
                  style={{ margin: "auto" }}
