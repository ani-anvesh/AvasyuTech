...
export default function Two2(props) {
  const data = props.data1.slice(0, 15);
  console.log(data);
  console.log(props);
  const theme = useTheme();
  const { id } = useParams();
  const maxSteps = data.length;
  const [number, setNumber] = useLocalStorage("QuestionNoOrange2", 0);
  const [activeStep, setActiveStep] = React.useState(parseInt(number));
  const [options, setOptions] = React.useState("");
  const [next, setNext] = React.useState(true);

  const [score, setScore] = useLocalStorage("scoreorange2", 0);
  const handleNext = () => {
