...
export default function Question3(props) {
  ...
  const [number, setNumber] = useLocalStorage("QuestionNo3", 0);
  const [activeStep, setActiveStep] = React.useState(parseInt(number));
  const [options, setOptions] = React.useState("");
  const [next, setNext] = React.useState(true);
  const [score, setScore] = useLocalStorage("score3", 0);
  const handleNext = () => {
  ...
  return (
    <div>
      <Paper className="root" style={{ maxHeight: "100%" }}>
        <MobileStepper
          variant="dots"
          steps={maxSteps}
          position="static"
          ...
          nextButton={
            <Link
              to={{
                pathname: `/question3/${activeStep + 1}`,
